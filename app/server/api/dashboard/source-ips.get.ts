...
interface SourceIpsResponse {
  ips: IpRow[]
}

export default defineEventHandler(async (event): Promise<SourceIpsResponse> => {
  ...
  const rows = await prisma.$queryRaw<
    Array<{ ip: string; country: string | null; total: number; passCount: number }>
  >`
    SELECT
      ar.sourceIp AS ip,
      gl.country,
    ...
    GROUP BY ar.sourceIp, gl.country
    ORDER BY total DESC
    LIMIT 10
  `
